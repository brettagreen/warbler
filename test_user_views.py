...
from models import db, connect_db, Message, User, Likes, Follows, PrivateAccountRequests
...
class MessageViewTestCase(TestCase):
    ...
    def test_user_show(self):
        with self.client as c:
            ...
            self.assertIn("You need to be logged in to perform that operation", str(resp.data))
    ...
    def test_add_like(self):
        ...
        with self.client as c:
            ...
            resp = c.post("/users/manage_likes/1984", follow_redirects=True)
            #self.assertEqual(resp.status_code, 200)
    ...
    def test_remove_like(self):
        ...
        with self.client as c:
            ...
            resp = c.post(f"/users/manage_likes/{m.id}", follow_redirects=True)
            #self.assertEqual(resp.status_code, 200)
    ...
    """def test_private_request_status(self):
        ...
        with self.client as c:
            with c.session_transaction() as sess:
                sess[CURR_USER_KEY] = self.testuser_id
            c.post('/users/follow/76543', follow_redirects=True)
            #self.assertEqual(resp.status_code, 200)
            sql = PrivateAccountRequests.query.get((self.testuser.id, 76543))
            self.assertEqual(sql.requesting_user_id, self.testuser.id)
            self.assertEqual(sql.private_user_id, 76543)
            self.assertTrue(sql.approved, False)"""
